...
close, no holdout-system-output access, acceptance conclusion, or claim of
independent experimental validation is authorized. Independent reference C/V/U
derivation from locked human annotations using separately reviewed,
non-production rules is required and permitted during reference preparation.
Production-system output must not supply or influence reference labels.
...
| **G6 — Annotation artifacts** | Use a versioned codebook and JSON Lines reference records, with primary annotations separated from adjudication records. Reference derivation and structural validation must be independent, reviewed, non-production implementations and must not call production detectors or calculators. | Versioned codebook; exact JSONL schema; primary/adjudication record definitions; annotation workflow/tool; limitation vocabulary; provenance/versioning rules; independent derivation/validator implementation, review, and non-evaluation validation evidence; then the completed primary, adjudication, derived-reference, hash, and validation artifacts. | **Checkpoint 1 — before holdout annotation begins:** researcher approval of the versioned codebook, JSONL schema, annotation workflow/tool, and independently reviewed non-production reference derivation/validator. **Checkpoint 2 — before reference freeze:** validation and researcher approval of the completed reference artifacts, including source-offset and reference-integrity results. |
